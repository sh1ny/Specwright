...
3. Implement deterministic refresh by recording `mtime`, size, and checksum for indexed source/test paths; `mtime` stores `stat.mtimeMs` in the existing wire shape. `scan --refresh` recomputes those fingerprints, reports missing/stale entries, and provides a fingerprint patch for the agent to apply after map sections are updated instead of persisting freshness immediately.
