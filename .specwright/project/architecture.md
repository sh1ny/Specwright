...
- `codebase-index.json` is versioned project intelligence: entrypoints, modules, commands, verification commands, risks, and fingerprints validated by `validateCodebaseIndex` (`src/core/validators.ts:100-308`).
...
- `scan` prepares project-level scan docs, ensures map/index artifacts, validates the index, and emits a bounded repo-inspection prompt.
