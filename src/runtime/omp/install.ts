...
export async function installOmpAdapter(input: { cwd: string; force: boolean }): Promise<string[]> {
  ...
  for (const [file, content] of Object.entries(AGENTS)) {
    writes.push([join(ompAgentsDir(input.cwd), file), content]);
  }
