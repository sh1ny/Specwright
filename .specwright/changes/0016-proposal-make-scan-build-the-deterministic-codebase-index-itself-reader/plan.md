...
Agents own only semantic prose: `scan.md`, `tech-stack.md`, `architecture.md`, and `codebase-map.md`. Prompts must never ask agents to author, paste, or hand-edit fingerprints. Evidence: `evidence.md` records that current `commandScan()` writes an empty default index and only refreshes under `--refresh` (`src/core/commands.ts:598-686`), current refresh cannot discover new files because it only checks already indexed paths (`src/core/commands.ts:440-509`), and the prompt currently asks agents to paste fingerprint JSON (`src/core/commands.ts:651-662`).
